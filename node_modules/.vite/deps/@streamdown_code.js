...
function resolveParsedThemeRules(parsedThemeRules, _colorMap) {
  parsedThemeRules.sort((a2, b3) => {
    let r5 = strcmp(a2.scope, b3.scope);
    if (r5 !== 0) {
      return r5;
    }
    r5 = strArrCmp(a2.parentScopes, b3.parentScopes);
    if (r5 !== 0) {
      return r5;
    }
...
var RuleFactory = class _RuleFactory {
  ...
  static _compileCaptures(captures, helper, repository) {
    ...
    if (captures) {
      ...
      for (const captureId in captures) {
        ...
        r5[numericCaptureId] = _RuleFactory.createCaptureRule(helper, captures[captureId].$vscodeTextmateLocation, captures[captureId].name, captures[captureId].contentName, retokenizeCapturedWithRuleId);
      }
    }
    return r5;
  }
  static _compilePatterns(patterns, helper, repository) {
    ...
    return {
      patterns: r5,
      hasMissingPatterns: (patterns ? patterns.length : 0) !== r5.length
    };
  }
};
...
var CompiledRule = class {
  ...
  toString() {
    ...
    for (let i2 = 0, len = this.rules.length; i2 < len; i2++) {
      r5.push("   - " + this.rules[i2] + ": " + this.regExps[i2]);
    }
    return r5.join("\n");
  }
...
function _tokenizeString(grammar, lineText, isFirstLine, linePos, stack, lineTokens, checkWhileConditions, timeLimit) {
  ...
  function scanNext() {
    ...
    const r5 = matchRuleOrInjections(
      grammar,
      lineText,
      isFirstLine,
      linePos,
    ...
    const captureIndices = r5.captureIndices;
    const matchedRuleId = r5.matchedRuleId;
...
function _checkWhileConditions(grammar, lineText, isFirstLine, linePos, stack, lineTokens) {
  ...
  for (let whileRule = whileRules.pop(); whileRule; whileRule = whileRules.pop()) {
    ...
    const r5 = ruleScanner.findNextMatchSync(lineText, linePos, findOptions);
    if (false) {
      console.log("  scanning for while rule");
      console.log(ruleScanner.toString());
    }
    if (r5) {
      const matchedRuleId = r5.ruleId;
      if (matchedRuleId !== whileRuleId) {
        stack = whileRule.stack.pop();
        break;
      }
      if (r5.captureIndices && r5.captureIndices.length) {
        lineTokens.produce(whileRule.stack, r5.captureIndices[0].start);
        handleCaptures(grammar, lineText, isFirstLine, whileRule.stack, lineTokens, whileRule.rule.whileCaptures, r5.captureIndices);
        lineTokens.produce(whileRule.stack, r5.captureIndices[0].end);
        anchorPosition = r5.captureIndices[0].end;
        if (r5.captureIndices[0].end > linePos) {
          linePos = r5.captureIndices[0].end;
          isFirstLine = false;
        }
      }
    } else {
...
function matchRule(grammar, lineText, isFirstLine, linePos, stack, anchorPosition) {
  ...
  if (r5) {
    return {
      captureIndices: r5.captureIndices,
      matchedRuleId: r5.ruleId
    };
  }
  return null;
}
...
var Grammar = class {
  ...
  tokenizeLine(lineText, prevState, timeLimit = 0) {
    const r5 = this._tokenize(lineText, prevState, false, timeLimit);
    return {
      tokens: r5.lineTokens.getResult(r5.ruleStack, r5.lineLength),
      ruleStack: r5.ruleStack,
      stoppedEarly: r5.stoppedEarly
    };
  }
  tokenizeLine2(lineText, prevState, timeLimit = 0) {
    const r5 = this._tokenize(lineText, prevState, true, timeLimit);
    return {
      tokens: r5.lineTokens.getBinaryResult(r5.ruleStack, r5.lineLength),
      ruleStack: r5.ruleStack,
      stoppedEarly: r5.stoppedEarly
    };
  }
...
function serializeAttribute(state, key2, value) {
  ...
  if (state.settings.preferUnquoted) {
    result = stringifyEntities(
      ...
      Object.assign({}, state.settings.characterReferences, {
        attribute: true,
        subset: constants.unquoted[x4][y3]
      })
    );
  }
  if (result !== value) {
    ...
    result = quote + stringifyEntities(
      ...
      Object.assign({}, state.settings.characterReferences, {
        // Always encode without parse errors in non-HTML.
        subset: (quote === "'" ? constants.single : constants.double)[x4][y3],
        attribute: true
      })
    ) + quote;
  }
...
function toArray(x4) {
  return Array.isArray(x4) ? x4 : [x4];
}
async function normalizeGetter(p3) {
  return Promise.resolve(typeof p3 === "function" ? p3() : p3).then((r5) => r5.default || r5);
}
...
function parseColor(sequence) {
  ...
  if (colorMode === "2") {
    const rgb = sequence.splice(0, 3).map((x4) => Number.parseInt(x4));
    if (rgb.length !== 3 || rgb.some((x4) => Number.isNaN(x4)))
      return;
    return {
      type: "rgb",
      rgb
...
function createColorPalette(namedColorsMap = defaultNamedColorsMap) {
  ...
  function rgbColor(rgb) {
    return `#${rgb.map((x4) => Math.max(0, Math.min(x4, 255)).toString(16).padStart(2, "0")).join("")}`;
  }
  let colorTable;
    ...
    for (let r5 = 0; r5 < 6; r5++) {
      for (let g = 0; g < 6; g++) {
        for (let b3 = 0; b3 < 6; b3++) {
          colorTable.push(rgbColor([levels[r5], levels[g], levels[b3]]));
        }
      }
    }
    let level = 8;
...
function dimColor(color) {
  ...
  if (hexMatch) {
    ...
    if (hex.length === 8) {
      ...
      return `#${r5}${r5}${g}${g}${b3}${b3}${alpha}`;
    } else if (hex.length === 3) {
      const r5 = hex[0];
      const g = hex[1];
      const b3 = hex[2];
      return `#${r5}${r5}${g}${g}${b3}${b3}80`;
    }
  }
...
function codeToTokensWithThemes(internal, code, options) {
  ...
  const mergedGrammarState = themedTokens[0].state ? new GrammarState(
    Object.fromEntries(themedTokens.map((s3) => {
      var _a4;
      return [s3.theme, (_a4 = s3.state) == null ? void 0 : _a4.getInternalStack(s3.theme)];
    })),
...
function normalizeTheme(rawTheme) {
  ...
  let { bg, fg } = theme;
  if (!bg || !fg) {
    const globalSetting = theme.settings ? theme.settings.find((s3) => !s3.name && !s3.scope) : void 0;
...
async function resolveLangs(langs) {
  return Array.from(new Set((await Promise.all(
    langs.filter((l3) => !isSpecialLang(l3)).map(async (lang) => await normalizeGetter(lang).then((r5) => Array.isArray(r5) ? r5 : [r5]))
  )).flat()));
}
...
var Resolver = class {
  constructor(engine, langs) {
    ...
    this._onigLib = {
      ...
      createOnigString: (s3) => engine.createString(s3)
    };
...
function makeSingletonHighlighterCore(createHighlighter2) {
  ...
  async function getSingletonHighlighterCore2(options) {
    if (!_shiki) {
      ...
      const s3 = await _shiki;
      await Promise.all([
        s3.loadTheme(...options.themes || []),
        s3.loadLanguage(...options.langs || [])
      ]);
      return s3;
    }
  }
...
var bundledLanguagesInfo = [
  {
    "id": "abap",
    "name": "ABAP",
    "import": (() => import("./abap-NSNLZLEJ.js"))
  },
  {
    "id": "actionscript-3",
    "name": "ActionScript",
    "import": (() => import("./actionscript-3-6SE5UUPK.js"))
  },
  {
    "id": "ada",
    "name": "Ada",
    "import": (() => import("./ada-EWS762WD.js"))
  },
  {
    "id": "angular-html",
    "name": "Angular HTML",
    "import": (() => import("./angular-html-AMXJXFDI.js"))
  },
  {
    "id": "angular-ts",
    "name": "Angular TypeScript",
    "import": (() => import("./angular-ts-TFVHDKQN.js"))
  },
  {
    "id": "apache",
    "name": "Apache Conf",
    "import": (() => import("./apache-OBAZ2RGQ.js"))
  },
  {
    "id": "apex",
    "name": "Apex",
    "import": (() => import("./apex-3XQBWEDZ.js"))
  },
  {
    "id": "apl",
    "name": "APL",
    "import": (() => import("./apl-MLTMBRW4.js"))
  },
  {
    "id": "applescript",
    "name": "AppleScript",
    "import": (() => import("./applescript-QT6XD5GB.js"))
  },
  {
    "id": "ara",
    "name": "Ara",
    "import": (() => import("./ara-LHZCSV4F.js"))
  },
  {
    "id": "asciidoc",
    "name": "AsciiDoc",
    ...
    "import": (() => import("./asciidoc-BY4BK2QI.js"))
  },
  {
    "id": "asm",
    "name": "Assembly",
    "import": (() => import("./asm-UVVRNLRU.js"))
  },
  {
    "id": "astro",
    "name": "Astro",
    "import": (() => import("./astro-YAJQL5QJ.js"))
  },
  {
    "id": "awk",
    "name": "AWK",
    "import": (() => import("./awk-YKPEWB4V.js"))
  },
  {
    "id": "ballerina",
    "name": "Ballerina",
    "import": (() => import("./ballerina-NHWTF5W7.js"))
  },
  {
    "id": "bat",
    "name": "Batch File",
    ...
    "import": (() => import("./bat-LP3SZ2PT.js"))
  },
  {
    "id": "beancount",
    "name": "Beancount",
    "import": (() => import("./beancount-JYCXB7IE.js"))
  },
  {
    "id": "berry",
    "name": "Berry",
    ...
    "import": (() => import("./berry-BUXS4QYA.js"))
  },
  {
    "id": "bibtex",
    "name": "BibTeX",
    "import": (() => import("./bibtex-M3GAKRLX.js"))
  },
  {
    "id": "bicep",
    "name": "Bicep",
    "import": (() => import("./bicep-UXJQ6CNU.js"))
  },
  {
    "id": "bird2",
    ...
    "import": (() => import("./bird2-LLUWAWSA.js"))
  },
  {
    "id": "blade",
    "name": "Blade",
    "import": (() => import("./blade-DTNRKBHU.js"))
  },
  {
    "id": "bsl",
    "name": "1C (Enterprise)",
    ...
    "import": (() => import("./bsl-RUYR4DMZ.js"))
  },
  {
    "id": "c",
    "name": "C",
    "import": (() => import("./c-WMZN6AHU.js"))
  },
  {
    "id": "c3",
    "name": "C3",
    "import": (() => import("./c3-33MO4OOZ.js"))
  },
  {
    "id": "cadence",
    "name": "Cadence",
    ...
    "import": (() => import("./cadence-Y2XEBPDZ.js"))
  },
  {
    "id": "cairo",
    "name": "Cairo",
    "import": (() => import("./cairo-4CKHIY7M.js"))
  },
  {
    "id": "clarity",
    "name": "Clarity",
    "import": (() => import("./clarity-GUEBPI2E.js"))
  },
  {
    "id": "clojure",
    "name": "Clojure",
    ...
    "import": (() => import("./clojure-JGWEA5WW.js"))
  },
  {
    "id": "cmake",
    "name": "CMake",
    "import": (() => import("./cmake-HM6BDJ7F.js"))
  },
  {
    "id": "cobol",
    "name": "COBOL",
    "import": (() => import("./cobol-LUXKBYHL.js"))
  },
  {
    "id": "codeowners",
    "name": "CODEOWNERS",
    "import": (() => import("./codeowners-7K2BLJTY.js"))
  },
  {
    "id": "codeql",
    "name": "CodeQL",
    ...
    "import": (() => import("./codeql-AAWBHV3B.js"))
  },
  {
    "id": "coffee",
    "name": "CoffeeScript",
    ...
    "import": (() => import("./coffee-M53Z6C6G.js"))
  },
  {
    "id": "common-lisp",
    "name": "Common Lisp",
    ...
    "import": (() => import("./common-lisp-PEPY25KB.js"))
  },
  {
    "id": "coq",
    "name": "Coq",
    "import": (() => import("./coq-PPJVNL26.js"))
  },
  {
    "id": "cpp",
    "name": "C++",
    ...
    "import": (() => import("./cpp-OLLP3LBP.js"))
  },
  {
    "id": "crystal",
    "name": "Crystal",
    "import": (() => import("./crystal-YCUYLYUI.js"))
  },
  {
    "id": "csharp",
    "name": "C#",
    ...
    "import": (() => import("./csharp-UKOO7AH2.js"))
  },
  {
    "id": "css",
    "name": "CSS",
    "import": (() => import("./css-HGBTECTT.js"))
  },
  {
    "id": "csv",
    "name": "CSV",
    "import": (() => import("./csv-WNKK766M.js"))
  },
  {
    "id": "cue",
    "name": "CUE",
    "import": (() => import("./cue-6CGFXUZP.js"))
  },
  {
    "id": "cypher",
    "name": "Cypher",
    ...
    "import": (() => import("./cypher-C4JSL7JW.js"))
  },
  {
    "id": "d",
    "name": "D",
    "import": (() => import("./d-PLUV7EF3.js"))
  },
  {
    "id": "dart",
    "name": "Dart",
    "import": (() => import("./dart-FRMH3667.js"))
  },
  {
    "id": "dax",
    "name": "DAX",
    "import": (() => import("./dax-6SG25E7M.js"))
  },
  {
    "id": "desktop",
    "name": "Desktop",
    "import": (() => import("./desktop-4QSLWIH5.js"))
  },
  {
    "id": "diff",
    "name": "Diff",
    "import": (() => import("./diff-FGFIK3BK.js"))
  },
  {
    "id": "docker",
    "name": "Dockerfile",
    ...
    "import": (() => import("./docker-44VPMKD6.js"))
  },
  {
    "id": "dotenv",
    "name": "dotEnv",
    "import": (() => import("./dotenv-GN4BPIJJ.js"))
  },
  {
    "id": "dream-maker",
    "name": "Dream Maker",
    "import": (() => import("./dream-maker-FHVF6POU.js"))
  },
  {
    "id": "edge",
    "name": "Edge",
    "import": (() => import("./edge-WTGH46A7.js"))
  },
  {
    "id": "elixir",
    "name": "Elixir",
    "import": (() => import("./elixir-6YKXGHRN.js"))
  },
  {
    "id": "elm",
    "name": "Elm",
    "import": (() => import("./elm-VZDQGYMM.js"))
  },
  {
    "id": "emacs-lisp",
    "name": "Emacs Lisp",
    ...
    "import": (() => import("./emacs-lisp-CL33V53U.js"))
  },
  {
    "id": "erb",
    "name": "ERB",
    "import": (() => import("./erb-LWOGSWHO.js"))
  },
  {
    "id": "erlang",
    "name": "Erlang",
    ...
    "import": (() => import("./erlang-WENIQIRV.js"))
  },
  {
    "id": "fennel",
    "name": "Fennel",
    "import": (() => import("./fennel-ODZ5BGCR.js"))
  },
  {
    "id": "fish",
    "name": "Fish",
    "import": (() => import("./fish-NWHYC4HE.js"))
  },
  {
    "id": "fluent",
    "name": "Fluent",
    ...
    "import": (() => import("./fluent-2OYHL2VP.js"))
  },
  {
    ...
    "import": (() => import("./fortran-fixed-form-QIB7SPIU.js"))
  },
  {
    ...
    "import": (() => import("./fortran-free-form-4GZW62LB.js"))
  },
  {
    "id": "fsharp",
    "name": "F#",
    ...
    "import": (() => import("./fsharp-YBWIMW5N.js"))
  },
  {
    "id": "gdresource",
    "name": "GDResource",
    ...
    "import": (() => import("./gdresource-KYWTO57R.js"))
  },
  {
    "id": "gdscript",
    "name": "GDScript",
    ...
    "import": (() => import("./gdscript-F563JJSK.js"))
  },
  {
    "id": "gdshader",
    "name": "GDShader",
    "import": (() => import("./gdshader-NCZFRFC5.js"))
  },
  {
    "id": "genie",
    "name": "Genie",
    "import": (() => import("./genie-T2WS7L7K.js"))
  },
  {
    "id": "gherkin",
    "name": "Gherkin",
    "import": (() => import("./gherkin-2VXOF4SW.js"))
  },
  {
    "id": "git-commit",
    "name": "Git Commit Message",
    "import": (() => import("./git-commit-JZ67CIPI.js"))
  },
  {
    "id": "git-rebase",
    "name": "Git Rebase Message",
    "import": (() => import("./git-rebase-FQWIQASU.js"))
  },
  {
    "id": "gleam",
    "name": "Gleam",
    "import": (() => import("./gleam-36VQQZWK.js"))
  },
  {
    "id": "glimmer-js",
    "name": "Glimmer JS",
    ...
    "import": (() => import("./glimmer-js-CECCDCHU.js"))
  },
  {
    "id": "glimmer-ts",
    "name": "Glimmer TS",
    ...
    "import": (() => import("./glimmer-ts-S54LJZCH.js"))
  },
  {
    "id": "glsl",
    "name": "GLSL",
    "import": (() => import("./glsl-VWHOYQM4.js"))
  },
  {
    "id": "gn",
    "name": "GN",
    "import": (() => import("./gn-IYWI7M7F.js"))
  },
  {
    "id": "gnuplot",
    "name": "Gnuplot",
    "import": (() => import("./gnuplot-DDVTXQOX.js"))
  },
  {
    "id": "go",
    "name": "Go",
    "import": (() => import("./go-P7BX3U4G.js"))
  },
  {
    "id": "graphql",
    "name": "GraphQL",
    ...
    "import": (() => import("./graphql-LXML5P33.js"))
  },
  {
    "id": "groovy",
    "name": "Groovy",
    "import": (() => import("./groovy-3I6PD4VF.js"))
  },
  {
    "id": "hack",
    "name": "Hack",
    "import": (() => import("./hack-PFTMIXQZ.js"))
  },
  {
    "id": "haml",
    "name": "Ruby Haml",
    "import": (() => import("./haml-6SODJTNA.js"))
  },
  {
    "id": "handlebars",
    "name": "Handlebars",
    ...
    "import": (() => import("./handlebars-MGBVO7ZE.js"))
  },
  {
    "id": "haskell",
    "name": "Haskell",
    ...
    "import": (() => import("./haskell-IQWZQIYZ.js"))
  },
  {
    "id": "haxe",
    "name": "Haxe",
    "import": (() => import("./haxe-A4NE7VCM.js"))
  },
  {
    "id": "hcl",
    "name": "HashiCorp HCL",
    "import": (() => import("./hcl-6DMX5XRA.js"))
  },
  {
    "id": "hjson",
    "name": "Hjson",
    "import": (() => import("./hjson-IVEWH2XO.js"))
  },
  {
    "id": "hlsl",
    "name": "HLSL",
    "import": (() => import("./hlsl-BOKNWPZN.js"))
  },
  {
    "id": "html",
    "name": "HTML",
    "import": (() => import("./html-UXTP3G2X.js"))
  },
  {
    "id": "html-derivative",
    "name": "HTML (Derivative)",
    "import": (() => import("./html-derivative-KP7PER5N.js"))
  },
  {
    "id": "http",
    "name": "HTTP",
    "import": (() => import("./http-QAL4JMEI.js"))
  },
  {
    "id": "hurl",
    "name": "Hurl",
    "import": (() => import("./hurl-ZNKYTE3I.js"))
  },
  {
    "id": "hxml",
    "name": "HXML",
    "import": (() => import("./hxml-KCHJZWZP.js"))
  },
  {
    "id": "hy",
    "name": "Hy",
    "import": (() => import("./hy-3GRVZBNR.js"))
  },
  {
    "id": "imba",
    "name": "Imba",
    "import": (() => import("./imba-FQXSZGS6.js"))
  },
  {
    "id": "ini",
    "name": "INI",
    ...
    "import": (() => import("./ini-IMC2MVYC.js"))
  },
  {
    "id": "java",
    "name": "Java",
    "import": (() => import("./java-BHHC6EK5.js"))
  },
  {
    "id": "javascript",
    "name": "JavaScript",
    ...
    "import": (() => import("./javascript-G75DL3LK.js"))
  },
  {
    "id": "jinja",
    "name": "Jinja",
    "import": (() => import("./jinja-XCZUVQVO.js"))
  },
  {
    "id": "jison",
    "name": "Jison",
    "import": (() => import("./jison-TO4DIH2U.js"))
  },
  {
    "id": "json",
    "name": "JSON",
    "import": (() => import("./json-552TAK4W.js"))
  },
  {
    "id": "json5",
    "name": "JSON5",
    "import": (() => import("./json5-NLNRFHTV.js"))
  },
  {
    "id": "jsonc",
    "name": "JSON with Comments",
    "import": (() => import("./jsonc-47GZROHN.js"))
  },
  {
    "id": "jsonl",
    "name": "JSON Lines",
    "import": (() => import("./jsonl-GLYQGQ7U.js"))
  },
  {
    "id": "jsonnet",
    "name": "Jsonnet",
    "import": (() => import("./jsonnet-GRCYNFR7.js"))
  },
  {
    "id": "jssm",
    "name": "JSSM",
    ...
    "import": (() => import("./jssm-NCE4O3CQ.js"))
  },
  {
    "id": "jsx",
    "name": "JSX",
    "import": (() => import("./jsx-PYDMLCWM.js"))
  },
  {
    "id": "julia",
    "name": "Julia",
    ...
    "import": (() => import("./julia-E2EV3SNG.js"))
  },
  {
    "id": "just",
    "name": "Just",
    "import": (() => import("./just-YQZCRQUV.js"))
  },
  {
    "id": "kdl",
    "name": "KDL",
    "import": (() => import("./kdl-XJEXZQ2Q.js"))
  },
  {
    "id": "kotlin",
    "name": "Kotlin",
    ...
    "import": (() => import("./kotlin-7ERZHUZC.js"))
  },
  {
    "id": "kusto",
    "name": "Kusto",
    ...
    "import": (() => import("./kusto-OYUVN5XN.js"))
  },
  {
    "id": "latex",
    "name": "LaTeX",
    "import": (() => import("./latex-NXNYEGDP.js"))
  },
  {
    "id": "lean",
    "name": "Lean 4",
    ...
    "import": (() => import("./lean-VR7KYF65.js"))
  },
  {
    "id": "less",
    "name": "Less",
    "import": (() => import("./less-V67JWA72.js"))
  },
  {
    "id": "liquid",
    "name": "Liquid",
    "import": (() => import("./liquid-DYQ3T76Z.js"))
  },
  {
    "id": "llvm",
    "name": "LLVM IR",
    "import": (() => import("./llvm-BUVL7QOO.js"))
  },
  {
    "id": "log",
    "name": "Log file",
    "import": (() => import("./log-S2HT5CGI.js"))
  },
  {
    "id": "logo",
    "name": "Logo",
    "import": (() => import("./logo-EXSRZQZG.js"))
  },
  {
    "id": "lua",
    "name": "Lua",
    "import": (() => import("./lua-KGT4PYI2.js"))
  },
  {
    "id": "luau",
    "name": "Luau",
    "import": (() => import("./luau-BKDAWIC7.js"))
  },
  {
    "id": "make",
    "name": "Makefile",
    ...
    "import": (() => import("./make-B7PMZHJP.js"))
  },
  {
    "id": "markdown",
    "name": "Markdown",
    ...
    "import": (() => import("./markdown-ZWHSJJLR.js"))
  },
  {
    "id": "marko",
    "name": "Marko",
    "import": (() => import("./marko-2YIBQSJN.js"))
  },
  {
    "id": "matlab",
    "name": "MATLAB",
    "import": (() => import("./matlab-2MPUVHST.js"))
  },
  {
    "id": "mdc",
    "name": "MDC",
    "import": (() => import("./mdc-W4MZGDTT.js"))
  },
  {
    "id": "mdx",
    "name": "MDX",
    "import": (() => import("./mdx-X5OLVQBB.js"))
  },
  {
    "id": "mermaid",
    "name": "Mermaid",
    ...
    "import": (() => import("./mermaid-4OEK6UQR.js"))
  },
  {
    "id": "mipsasm",
    "name": "MIPS Assembly",
    ...
    "import": (() => import("./mipsasm-7ZFE5RUK.js"))
  },
  {
    "id": "mojo",
    "name": "Mojo",
    "import": (() => import("./mojo-ZP2KWMIT.js"))
  },
  {
    "id": "moonbit",
    "name": "MoonBit",
    ...
    "import": (() => import("./moonbit-EHULIXVR.js"))
  },
  {
    "id": "move",
    "name": "Move",
    "import": (() => import("./move-BCLTMGS7.js"))
  },
  {
    "id": "narrat",
    ...
    "import": (() => import("./narrat-6GJYX65D.js"))
  },
  {
    "id": "nextflow",
    "name": "Nextflow",
    ...
    "import": (() => import("./nextflow-ETRQBSEB.js"))
  },
  {
    "id": "nextflow-groovy",
    "name": "nextflow-groovy",
    "import": (() => import("./nextflow-groovy-WU77462U.js"))
  },
  {
    "id": "nginx",
    "name": "Nginx",
    "import": (() => import("./nginx-SID7Q3GC.js"))
  },
  {
    "id": "nim",
    "name": "Nim",
    "import": (() => import("./nim-4PZOUDXG.js"))
  },
  {
    "id": "nix",
    "name": "Nix",
    "import": (() => import("./nix-J5ERUNFB.js"))
  },
  {
    "id": "nushell",
    "name": "nushell",
    ...
    "import": (() => import("./nushell-EO66LO6Y.js"))
  },
  {
    "id": "objective-c",
    "name": "Objective-C",
    ...
    "import": (() => import("./objective-c-CDVZVV2Q.js"))
  },
  {
    "id": "objective-cpp",
    "name": "Objective-C++",
    "import": (() => import("./objective-cpp-GDXJL7ZL.js"))
  },
  {
    "id": "ocaml",
    "name": "OCaml",
    "import": (() => import("./ocaml-U5IAWEJ7.js"))
  },
  {
    "id": "odin",
    "name": "Odin",
    "import": (() => import("./odin-GTDECJAQ.js"))
  },
  {
    "id": "openscad",
    "name": "OpenSCAD",
    ...
    "import": (() => import("./openscad-76MMRGNB.js"))
  },
  {
    "id": "pascal",
    "name": "Pascal",
    "import": (() => import("./pascal-TSTPLVC2.js"))
  },
  {
    "id": "perl",
    "name": "Perl",
    "import": (() => import("./perl-6F2JSTEU.js"))
  },
  {
    "id": "php",
    "name": "PHP",
    "import": (() => import("./php-76SEYYHE.js"))
  },
  {
    "id": "pkl",
    "name": "Pkl",
    "import": (() => import("./pkl-OUTUEHPF.js"))
  },
  {
    "id": "plsql",
    "name": "PL/SQL",
    "import": (() => import("./plsql-ABJBLE7S.js"))
  },
  {
    "id": "po",
    "name": "Gettext PO",
    ...
    "import": (() => import("./po-4477PJPM.js"))
  },
  {
    "id": "polar",
    "name": "Polar",
    "import": (() => import("./polar-B24BKN6D.js"))
  },
  {
    "id": "postcss",
    "name": "PostCSS",
    "import": (() => import("./postcss-J7YNVODT.js"))
  },
  {
    "id": "powerquery",
    "name": "PowerQuery",
    "import": (() => import("./powerquery-XDQYQAES.js"))
  },
  {
    "id": "powershell",
    "name": "PowerShell",
    ...
    "import": (() => import("./powershell-7TNFCVEC.js"))
  },
  {
    "id": "prisma",
    "name": "Prisma",
    "import": (() => import("./prisma-CJAV2IPO.js"))
  },
  {
    "id": "prolog",
    "name": "Prolog",
    "import": (() => import("./prolog-VOAY3KMI.js"))
  },
  {
    "id": "proto",
    ...
    "import": (() => import("./proto-2RDORE5H.js"))
  },
  {
    "id": "pug",
    "name": "Pug",
    ...
    "import": (() => import("./pug-2EWEVYNA.js"))
  },
  {
    "id": "puppet",
    "name": "Puppet",
    "import": (() => import("./puppet-YUJHYBYL.js"))
  },
  {
    "id": "purescript",
    "name": "PureScript",
    "import": (() => import("./purescript-JG3P45H5.js"))
  },
  {
    "id": "python",
    "name": "Python",
    ...
    "import": (() => import("./python-N47GUV3L.js"))
  },
  {
    "id": "qml",
    "name": "QML",
    "import": (() => import("./qml-BRVTTV7M.js"))
  },
  {
    "id": "qmldir",
    "name": "QML Directory",
    "import": (() => import("./qmldir-EBWPLYU6.js"))
  },
  {
    "id": "qss",
    "name": "Qt Style Sheets",
    "import": (() => import("./qss-ER4ZKDCB.js"))
  },
  {
    "id": "r",
    "name": "R",
    "import": (() => import("./r-34DIVFCG.js"))
  },
  {
    "id": "racket",
    "name": "Racket",
    "import": (() => import("./racket-AZDC6WZP.js"))
  },
  {
    "id": "raku",
    "name": "Raku",
    ...
    "import": (() => import("./raku-KJ3HQN4L.js"))
  },
  {
    "id": "razor",
    "name": "ASP.NET Razor",
    "import": (() => import("./razor-FZRHTLZA.js"))
  },
  {
    "id": "reg",
    "name": "Windows Registry Script",
    "import": (() => import("./reg-3FBDQXG2.js"))
  },
  {
    "id": "regexp",
    "name": "RegExp",
    ...
    "import": (() => import("./regexp-XLDLE3EI.js"))
  },
  {
    "id": "rel",
    "name": "Rel",
    "import": (() => import("./rel-H7DSYWZF.js"))
  },
  {
    "id": "riscv",
    "name": "RISC-V",
    "import": (() => import("./riscv-PWMUGY2F.js"))
  },
  {
    "id": "ron",
    "name": "RON",
    "import": (() => import("./ron-TFJWE6P5.js"))
  },
  {
    "id": "rosmsg",
    "name": "ROS Interface",
    "import": (() => import("./rosmsg-A332KOGQ.js"))
  },
  {
    "id": "rst",
    "name": "reStructuredText",
    "import": (() => import("./rst-LKIWNMU6.js"))
  },
  {
    "id": "ruby",
    "name": "Ruby",
    ...
    "import": (() => import("./ruby-HBHWCEBY.js"))
  },
  {
    "id": "rust",
    "name": "Rust",
    ...
    "import": (() => import("./rust-R6RXHTJQ.js"))
  },
  {
    "id": "sas",
    "name": "SAS",
    "import": (() => import("./sas-4RBPZ5VC.js"))
  },
  {
    "id": "sass",
    "name": "Sass",
    "import": (() => import("./sass-47LK7SIU.js"))
  },
  {
    "id": "scala",
    "name": "Scala",
    "import": (() => import("./scala-4SMCPYMM.js"))
  },
  {
    "id": "scheme",
    "name": "Scheme",
    "import": (() => import("./scheme-3OA5G6TI.js"))
  },
  {
    "id": "scss",
    "name": "SCSS",
    "import": (() => import("./scss-PLD7FVCO.js"))
  },
  {
    "id": "sdbl",
    "name": "1C (Query)",
    ...
    "import": (() => import("./sdbl-GHP5RT3S.js"))
  },
  {
    "id": "shaderlab",
    "name": "ShaderLab",
    ...
    "import": (() => import("./shaderlab-PM5JPFNH.js"))
  },
  {
    "id": "shellscript",
    "name": "Shell",
    ...
    "import": (() => import("./shellscript-FLSFUFRJ.js"))
  },
  {
    "id": "shellsession",
    "name": "Shell Session",
    ...
    "import": (() => import("./shellsession-BVA5MHXS.js"))
  },
  {
    "id": "smalltalk",
    "name": "Smalltalk",
    "import": (() => import("./smalltalk-VN4J63SC.js"))
  },
  {
    "id": "solidity",
    "name": "Solidity",
    "import": (() => import("./solidity-VNXH4C4G.js"))
  },
  {
    "id": "soy",
    ...
    "import": (() => import("./soy-TNOGVCH4.js"))
  },
  {
    "id": "sparql",
    "name": "SPARQL",
    "import": (() => import("./sparql-MI2C27PM.js"))
  },
  {
    "id": "splunk",
    ...
    "import": (() => import("./splunk-CIRWTYCO.js"))
  },
  {
    "id": "sql",
    "name": "SQL",
    "import": (() => import("./sql-WOMUWTFK.js"))
  },
  {
    "id": "ssh-config",
    "name": "SSH Config",
    "import": (() => import("./ssh-config-3YUXGXTH.js"))
  },
  {
    "id": "stata",
    "name": "Stata",
    "import": (() => import("./stata-M77S2UKZ.js"))
  },
  {
    "id": "stylus",
    "name": "Stylus",
    ...
    "import": (() => import("./stylus-S7GPX36R.js"))
  },
  {
    "id": "surrealql",
    "name": "SurrealQL",
    ...
    "import": (() => import("./surrealql-FSHPIPEG.js"))
  },
  {
    "id": "svelte",
    "name": "Svelte",
    "import": (() => import("./svelte-OH47UKFA.js"))
  },
  {
    "id": "swift",
    "name": "Swift",
    "import": (() => import("./swift-OYOVB2FK.js"))
  },
  {
    "id": "system-verilog",
    "name": "SystemVerilog",
    "import": (() => import("./system-verilog-LYGWOAWC.js"))
  },
  {
    "id": "systemd",
    "name": "Systemd Units",
    "import": (() => import("./systemd-TBN4KXXW.js"))
  },
  {
    "id": "talonscript",
    "name": "TalonScript",
    ...
    "import": (() => import("./talonscript-6I55KMUQ.js"))
  },
  {
    "id": "tasl",
    "name": "Tasl",
    "import": (() => import("./tasl-UE3CQECZ.js"))
  },
  {
    "id": "tcl",
    "name": "Tcl",
    "import": (() => import("./tcl-JXPA5K4E.js"))
  },
  {
    "id": "templ",
    "name": "Templ",
    "import": (() => import("./templ-TLB2MVLO.js"))
  },
  {
    "id": "terraform",
    "name": "Terraform",
    ...
    "import": (() => import("./terraform-YYDKU7DH.js"))
  },
  {
    "id": "tex",
    "name": "TeX",
    "import": (() => import("./tex-SIZSPJVD.js"))
  },
  {
    "id": "toml",
    "name": "TOML",
    "import": (() => import("./toml-WRO5EL6R.js"))
  },
  {
    "id": "ts-tags",
    ...
    "import": (() => import("./ts-tags-FQLVNWHY.js"))
  },
  {
    "id": "tsv",
    "name": "TSV",
    "import": (() => import("./tsv-5WQVSBLQ.js"))
  },
  {
    "id": "tsx",
    "name": "TSX",
    "import": (() => import("./tsx-3XU2MPAE.js"))
  },
  {
    "id": "turtle",
    "name": "Turtle",
    "import": (() => import("./turtle-IW63XP5W.js"))
  },
  {
    "id": "twig",
    "name": "Twig",
    "import": (() => import("./twig-62H44IJF.js"))
  },
  {
    "id": "typescript",
    "name": "TypeScript",
    ...
    "import": (() => import("./typescript-A7T4NTPG.js"))
  },
  {
    "id": "typespec",
    "name": "TypeSpec",
    ...
    "import": (() => import("./typespec-6DXNM2BT.js"))
  },
  {
    "id": "typst",
    "name": "Typst",
    ...
    "import": (() => import("./typst-ZSRU3XH5.js"))
  },
  {
    "id": "v",
    "name": "V",
    "import": (() => import("./v-7W2TV4EV.js"))
  },
  {
    "id": "vala",
    "name": "Vala",
    "import": (() => import("./vala-35MQIRDW.js"))
  },
  {
    "id": "vb",
    "name": "Visual Basic",
    ...
    "import": (() => import("./vb-QUDLXVGO.js"))
  },
  {
    "id": "verilog",
    "name": "Verilog",
    "import": (() => import("./verilog-KIIOVAJE.js"))
  },
  {
    "id": "vhdl",
    "name": "VHDL",
    "import": (() => import("./vhdl-G3KACLOD.js"))
  },
  {
    "id": "viml",
    "name": "Vim Script",
    ...
    "import": (() => import("./viml-WD7KTPZM.js"))
  },
  {
    "id": "vue",
    "name": "Vue",
    "import": (() => import("./vue-RZMOAKGB.js"))
  },
  {
    "id": "vue-html",
    "name": "Vue HTML",
    "import": (() => import("./vue-html-HO5TISWO.js"))
  },
  {
    "id": "vue-vine",
    "name": "Vue Vine",
    "import": (() => import("./vue-vine-PGCYVSI4.js"))
  },
  {
    "id": "vyper",
    "name": "Vyper",
    ...
    "import": (() => import("./vyper-UQLGF62Y.js"))
  },
  {
    "id": "wasm",
    "name": "WebAssembly",
    "import": (() => import("./wasm-4YTRY4FR.js"))
  },
  {
    "id": "wenyan",
    "name": "Wenyan",
    ...
    "import": (() => import("./wenyan-SPOHXZWD.js"))
  },
  {
    "id": "wgsl",
    "name": "WGSL",
    "import": (() => import("./wgsl-5Z22WII2.js"))
  },
  {
    "id": "wikitext",
    "name": "Wikitext",
    ...
    "import": (() => import("./wikitext-2Y7S6AUT.js"))
  },
  {
    "id": "wit",
    "name": "WebAssembly Interface Types",
    "import": (() => import("./wit-56UPOJBI.js"))
  },
  {
    "id": "wolfram",
    "name": "Wolfram",
    ...
    "import": (() => import("./wolfram-R6PH6P6W.js"))
  },
  {
    "id": "xml",
    "name": "XML",
    "import": (() => import("./xml-C556GLFJ.js"))
  },
  {
    "id": "xsl",
    "name": "XSL",
    "import": (() => import("./xsl-DFAQNXWR.js"))
  },
  {
    "id": "yaml",
    "name": "YAML",
    ...
    "import": (() => import("./yaml-JN6XHGBS.js"))
  },
  {
    "id": "zenscript",
    "name": "ZenScript",
    "import": (() => import("./zenscript-YBFJD3OZ.js"))
  },
  {
    "id": "zig",
    "name": "Zig",
    "import": (() => import("./zig-4WCX4RX3.js"))
  }
];
...
var bundledThemesInfo = [
  {
    ...
    "type": "dark",
    "import": (() => import("./andromeeda-XBT7HSHM.js"))
  },
  {
    "id": "aurora-x",
    "displayName": "Aurora X",
    "type": "dark",
    "import": (() => import("./aurora-x-M4EV7ZZZ.js"))
  },
  {
    "id": "ayu-dark",
    "displayName": "Ayu Dark",
    "type": "dark",
    "import": (() => import("./ayu-dark-BIZPEBVG.js"))
  },
  {
    "id": "ayu-light",
    "displayName": "Ayu Light",
    "type": "light",
    "import": (() => import("./ayu-light-ROWCICRX.js"))
  },
  {
    "id": "ayu-mirage",
    "displayName": "Ayu Mirage",
    "type": "dark",
    "import": (() => import("./ayu-mirage-IGLUTRIU.js"))
  },
  {
    "id": "catppuccin-frappe",
    "displayName": "Catppuccin Frappé",
    "type": "dark",
    "import": (() => import("./catppuccin-frappe-4WUWXJGN.js"))
  },
  {
    "id": "catppuccin-latte",
    "displayName": "Catppuccin Latte",
    "type": "light",
    "import": (() => import("./catppuccin-latte-GRQLFFUR.js"))
  },
  {
    "id": "catppuccin-macchiato",
    "displayName": "Catppuccin Macchiato",
    "type": "dark",
    "import": (() => import("./catppuccin-macchiato-PGDKT3NO.js"))
  },
  {
    "id": "catppuccin-mocha",
    "displayName": "Catppuccin Mocha",
    "type": "dark",
    "import": (() => import("./catppuccin-mocha-KM7JEXAD.js"))
  },
  {
    "id": "dark-plus",
    "displayName": "Dark Plus",
    "type": "dark",
    "import": (() => import("./dark-plus-UHLIXWT7.js"))
  },
  {
    "id": "dracula",
    "displayName": "Dracula Theme",
    "type": "dark",
    "import": (() => import("./dracula-AY6J5QXZ.js"))
  },
  {
    "id": "dracula-soft",
    "displayName": "Dracula Theme Soft",
    "type": "dark",
    "import": (() => import("./dracula-soft-FW3OB2YL.js"))
  },
  {
    "id": "everforest-dark",
    "displayName": "Everforest Dark",
    "type": "dark",
    "import": (() => import("./everforest-dark-UMQNCFI3.js"))
  },
  {
    "id": "everforest-light",
    "displayName": "Everforest Light",
    "type": "light",
    "import": (() => import("./everforest-light-ZS2DGNHW.js"))
  },
  {
    "id": "github-dark",
    "displayName": "GitHub Dark",
    "type": "dark",
    "import": (() => import("./github-dark-SOXV4YLB.js"))
  },
  {
    "id": "github-dark-default",
    "displayName": "GitHub Dark Default",
    "type": "dark",
    "import": (() => import("./github-dark-default-EBGHOZTC.js"))
  },
  {
    "id": "github-dark-dimmed",
    "displayName": "GitHub Dark Dimmed",
    "type": "dark",
    "import": (() => import("./github-dark-dimmed-2P3VTT75.js"))
  },
  {
    "id": "github-dark-high-contrast",
    "displayName": "GitHub Dark High Contrast",
    "type": "dark",
    "import": (() => import("./github-dark-high-contrast-H2T2V4Q6.js"))
  },
  {
    "id": "github-light",
    "displayName": "GitHub Light",
    "type": "light",
    "import": (() => import("./github-light-HVTMXB7B.js"))
  },
  {
    "id": "github-light-default",
    "displayName": "GitHub Light Default",
    "type": "light",
    "import": (() => import("./github-light-default-CD2SAL3M.js"))
  },
  {
    "id": "github-light-high-contrast",
    "displayName": "GitHub Light High Contrast",
    "type": "light",
    "import": (() => import("./github-light-high-contrast-IVCSPFW3.js"))
  },
  {
    "id": "gruvbox-dark-hard",
    "displayName": "Gruvbox Dark Hard",
    "type": "dark",
    "import": (() => import("./gruvbox-dark-hard-2322ZBXD.js"))
  },
  {
    "id": "gruvbox-dark-medium",
    "displayName": "Gruvbox Dark Medium",
    "type": "dark",
    "import": (() => import("./gruvbox-dark-medium-2H6MBUHI.js"))
  },
  {
    "id": "gruvbox-dark-soft",
    "displayName": "Gruvbox Dark Soft",
    "type": "dark",
    "import": (() => import("./gruvbox-dark-soft-SOQCT5JJ.js"))
  },
  {
    "id": "gruvbox-light-hard",
    "displayName": "Gruvbox Light Hard",
    "type": "light",
    "import": (() => import("./gruvbox-light-hard-HUIUAYOC.js"))
  },
  {
    "id": "gruvbox-light-medium",
    "displayName": "Gruvbox Light Medium",
    "type": "light",
    "import": (() => import("./gruvbox-light-medium-3TF4II73.js"))
  },
  {
    "id": "gruvbox-light-soft",
    "displayName": "Gruvbox Light Soft",
    "type": "light",
    "import": (() => import("./gruvbox-light-soft-SB4Q47N7.js"))
  },
  {
    "id": "horizon",
    "displayName": "Horizon",
    "type": "dark",
    "import": (() => import("./horizon-Z32BCLIX.js"))
  },
  {
    "id": "horizon-bright",
    "displayName": "Horizon Bright",
    "type": "dark",
    "import": (() => import("./horizon-bright-63DJSME6.js"))
  },
  {
    "id": "houston",
    "displayName": "Houston",
    "type": "dark",
    "import": (() => import("./houston-6FTQNGJA.js"))
  },
  {
    "id": "kanagawa-dragon",
    "displayName": "Kanagawa Dragon",
    "type": "dark",
    "import": (() => import("./kanagawa-dragon-4HJNFITS.js"))
  },
  {
    "id": "kanagawa-lotus",
    "displayName": "Kanagawa Lotus",
    "type": "light",
    "import": (() => import("./kanagawa-lotus-UZ5PCS2N.js"))
  },
  {
    "id": "kanagawa-wave",
    "displayName": "Kanagawa Wave",
    "type": "dark",
    "import": (() => import("./kanagawa-wave-AGVCF5YC.js"))
  },
  {
    "id": "laserwave",
    "displayName": "LaserWave",
    "type": "dark",
    "import": (() => import("./laserwave-3BKTSZF4.js"))
  },
  {
    "id": "light-plus",
    "displayName": "Light Plus",
    "type": "light",
    "import": (() => import("./light-plus-T36DV6Y6.js"))
  },
  {
    "id": "material-theme",
    "displayName": "Material Theme",
    "type": "dark",
    "import": (() => import("./material-theme-7HVT4HE5.js"))
  },
  {
    "id": "material-theme-darker",
    "displayName": "Material Theme Darker",
    "type": "dark",
    "import": (() => import("./material-theme-darker-M72G3E3K.js"))
  },
  {
    "id": "material-theme-lighter",
    "displayName": "Material Theme Lighter",
    "type": "light",
    "import": (() => import("./material-theme-lighter-BHFQRDJE.js"))
  },
  {
    "id": "material-theme-ocean",
    "displayName": "Material Theme Ocean",
    "type": "dark",
    "import": (() => import("./material-theme-ocean-PC3GMVRD.js"))
  },
  {
    "id": "material-theme-palenight",
    "displayName": "Material Theme Palenight",
    "type": "dark",
    "import": (() => import("./material-theme-palenight-R7PZE7BS.js"))
  },
  {
    "id": "min-dark",
    "displayName": "Min Dark",
    "type": "dark",
    "import": (() => import("./min-dark-7EZAVTUY.js"))
  },
  {
    "id": "min-light",
    "displayName": "Min Light",
    "type": "light",
    "import": (() => import("./min-light-DMC2P5SP.js"))
  },
  {
    "id": "monokai",
    "displayName": "Monokai",
    "type": "dark",
    "import": (() => import("./monokai-2CZP553K.js"))
  },
  {
    "id": "night-owl",
    "displayName": "Night Owl",
    "type": "dark",
    "import": (() => import("./night-owl-WYMMDDZW.js"))
  },
  {
    "id": "night-owl-light",
    "displayName": "Night Owl Light",
    "type": "light",
    "import": (() => import("./night-owl-light-F2GEDTXO.js"))
  },
  {
    "id": "nord",
    "displayName": "Nord",
    "type": "dark",
    "import": (() => import("./nord-F3GHLXZV.js"))
  },
  {
    "id": "one-dark-pro",
    "displayName": "One Dark Pro",
    "type": "dark",
    "import": (() => import("./one-dark-pro-IIP2SQXB.js"))
  },
  {
    "id": "one-light",
    "displayName": "One Light",
    "type": "light",
    "import": (() => import("./one-light-5BNGLPSD.js"))
  },
  {
    "id": "plastic",
    "displayName": "Plastic",
    "type": "dark",
    "import": (() => import("./plastic-4P33GBHN.js"))
  },
  {
    "id": "poimandres",
    "displayName": "Poimandres",
    "type": "dark",
    "import": (() => import("./poimandres-FHO5S75Z.js"))
  },
  {
    "id": "red",
    "displayName": "Red",
    "type": "dark",
    "import": (() => import("./red-EUEVQ7JT.js"))
  },
  {
    "id": "rose-pine",
    "displayName": "Rosé Pine",
    "type": "dark",
    "import": (() => import("./rose-pine-VLTGXESQ.js"))
  },
  {
    "id": "rose-pine-dawn",
    "displayName": "Rosé Pine Dawn",
    "type": "light",
    "import": (() => import("./rose-pine-dawn-3QNDUA5H.js"))
  },
  {
    "id": "rose-pine-moon",
    "displayName": "Rosé Pine Moon",
    "type": "dark",
    "import": (() => import("./rose-pine-moon-G5R457SI.js"))
  },
  {
    "id": "slack-dark",
    "displayName": "Slack Dark",
    "type": "dark",
    "import": (() => import("./slack-dark-PRT4TPNM.js"))
  },
  {
    "id": "slack-ochin",
    "displayName": "Slack Ochin",
    "type": "light",
    "import": (() => import("./slack-ochin-Z4XZKPOY.js"))
  },
  {
    "id": "snazzy-light",
    "displayName": "Snazzy Light",
    "type": "light",
    "import": (() => import("./snazzy-light-DNA4JKOL.js"))
  },
  {
    "id": "solarized-dark",
    "displayName": "Solarized Dark",
    "type": "dark",
    "import": (() => import("./solarized-dark-36BVI5BK.js"))
  },
  {
    "id": "solarized-light",
    "displayName": "Solarized Light",
    "type": "light",
    "import": (() => import("./solarized-light-ZDVDG25G.js"))
  },
  {
    "id": "synthwave-84",
    "displayName": "Synthwave '84",
    "type": "dark",
    "import": (() => import("./synthwave-84-JQLGXJ2F.js"))
  },
  {
    "id": "tokyo-night",
    "displayName": "Tokyo Night",
    "type": "dark",
    "import": (() => import("./tokyo-night-HYLSI4LE.js"))
  },
  {
    "id": "vesper",
    "displayName": "Vesper",
    "type": "dark",
    "import": (() => import("./vesper-TVPOQR5K.js"))
  },
  {
    "id": "vitesse-black",
    "displayName": "Vitesse Black",
    "type": "dark",
    "import": (() => import("./vitesse-black-ZJNPD7S5.js"))
  },
  {
    "id": "vitesse-dark",
    "displayName": "Vitesse Dark",
    "type": "dark",
    "import": (() => import("./vitesse-dark-LG6NNKSM.js"))
  },
  {
    "id": "vitesse-light",
    "displayName": "Vitesse Light",
    "type": "light",
    "import": (() => import("./vitesse-light-EB7M3ZEV.js"))
  }
];
...
var alignUp = (x4, multiple) => x4 + (multiple - x4 % multiple) % multiple;
...
var createHighlighter = createBundledHighlighter({
  ...
  engine: () => createOnigurumaEngine(import("./wasm-T2ZVUQMM.js"))
});
var {
...
function M(e, n = {}) {
  ...
  const o4 = Y(t.flags), s3 = [o4.extended], a2 = { captureGroup: t.rules.captureGroup, getCurrentModX() {
    return s3.at(-1);
  ...
  for (C.lastIndex = 0; i2 = C.exec(e); ) {
    ...
    u2.tokens ? r5.push(...u2.tokens) : u2.token && r5.push(u2.token), u2.lastIndex !== void 0 && (C.lastIndex = u2.lastIndex);
  }
  const l3 = [];
  let c2 = 0;
  r5.filter((u2) => u2.type === "GroupOpen").forEach((u2) => {
    u2.kind === "capturing" ? u2.number = ++c2 : u2.raw === "(" && l3.push(u2);
  }), c2 || l3.forEach((u2, S3) => {
    u2.kind = "capturing", u2.number = S3 + 1;
  });
  const g = c2 || l3.length;
  return { tokens: r5.map((u2) => u2.type === "EscapedNumber" ? ee(u2, g) : u2).flat(), flags: o4 };
}
function F(e, n, t, o4) {
  const [s3, a2] = t;
  if (t === "[" || t === "[^") {
    const r5 = K(n, t, o4);
    return { tokens: r5.tokens, lastIndex: r5.lastIndex };
  }
  if (s3 === "\\") {
    ...
    const r5 = x(t, { inCharClass: false });
    return Array.isArray(r5) ? { tokens: r5 } : { token: r5 };
  }
  if (s3 === "(") {
    if (a2 === "*") return { token: j(t) };
    if (t === "(?{") throw new Error(`Unsupported callout "${t}"`);
    if (t.startsWith("(?#")) {
      if (n[o4] !== ")") throw new Error('Unclosed comment group "(?#"');
      return { lastIndex: o4 + 1 };
    }
  ...
  if (e.getCurrentModX()) {
    if (t === "#") {
      const r5 = n.indexOf(`
`, o4);
      return { lastIndex: r5 === -1 ? n.length : r5 };
    }
    if (/^\s$/.test(t)) {
      const r5 = /\s+/y;
      return r5.lastIndex = o4, { lastIndex: r5.exec(n) ? r5.lastIndex : o4 };
    }
  }
  if (t === ".") return { token: k("dot", t) };
  if (t === "^" || t === "$") {
    const r5 = e.singleline ? { "^": o`\A`, $: o`\Z` }[t] : t;
    return { token: w(r5, t) };
  }
...
function K(e, n, t) {
  ...
  for (T.lastIndex = t; a2 = T.exec(e); ) {
    const r5 = a2[0];
    if (r5[0] === "[" && r5[1] !== ":") s3++, o4.push(E(r5[1] === "^", r5));
    else if (r5 === "]") {
      if (o4.at(-1).type === "CharacterClassOpen") o4.push(d(93, r5));
      else if (s3--, o4.push(z(r5)), !s3) break;
    } else {
      const i2 = X(r5);
      Array.isArray(i2) ? o4.push(...i2) : o4.push(i2);
    }
  }
  return { tokens: o4, lastIndex: T.lastIndex || e.length };
}
function X(e) {
...
function x(e, { inCharClass: n }) {
  ...
  if (new RegExp("^\\\\x[89A-Fa-f]\\p{AHex}", "u").test(e)) try {
    const o4 = e.split(/\\x/).slice(1).map((i2) => parseInt(i2, 16)), s3 = new TextDecoder("utf-8", { ignoreBOM: true, fatal: true }).decode(new Uint8Array(o4)), a2 = new TextEncoder();
    return [...s3].map((i2) => {
      const l3 = [...a2.encode(i2)].map((c2) => `\\x${c2.toString(16)}`).join("");
      return d(r(i2), l3);
    });
  } catch {
...
function D(e, n, t, o4) {
  return { type: "NamedCallout", kind: e, tag: n, arguments: t, raw: o4 };
}
function _(e, n, t, o4) {
  return { type: "Quantifier", kind: e, min: n, max: t, raw: o4 };
}
function R(e) {
...
function L(e, n) {
  let { on: t, off: o4 } = /^\(\?(?<on>[imx]*)(?:-(?<off>[-imx]*))?/.exec(e).groups;
  o4 ?? (o4 = "");
  const s3 = (n.getCurrentModX() || t.includes("x")) && !o4.includes("x"), a2 = v(t), r5 = v(o4), i2 = {};
  if (a2 && (i2.enable = a2), r5 && (i2.disable = r5), e.endsWith(")")) return n.replaceCurrentModX(s3), I("flags", e, { flags: i2 });
  if (e.endsWith(":")) return n.pushModX(s3), n.numOpenGroups++, f("group", e, { ...(a2 || r5) && { flags: i2 } });
...
function j(e) {
  ...
  const { name: t, tag: o4, args: s3 } = n.groups;
  if (!t) throw new Error(`Invalid named callout "${e}"`);
  if (o4 === "") throw new Error(`Named callout tag with empty value not allowed "${e}"`);
  const a2 = s3 ? s3.split(",").filter((g) => g !== "").map((g) => /^[+-]?\d+$/.test(g) ? +g : g) : [], [r5, i2, l3] = a2, c2 = B.has(t) ? t.toLowerCase() : "custom";
  switch (c2) {
    case "fail":
    case "mismatch":
    case "skip":
    ...
    case "error":
      ...
      if (typeof r5 == "string") throw new Error(`Named callout argument must be a number "${r5}"`);
      break;
    case "max":
      if (!a2.length || a2.length > 2) throw new Error(`Named callout must have one or two arguments "${a2}"`);
      if (typeof r5 == "string" && !/^[A-Za-z_]\w*$/.test(r5)) throw new Error(`Named callout argument one must be a tag or number "${r5}"`);
      if (a2.length === 2 && (typeof i2 == "number" || !/^[<>X]$/.test(i2))) throw new Error(`Named callout optional argument two must be '<', '>', or 'X' "${i2}"`);
      break;
    case "count":
    case "total_count":
      if (a2.length > 1) throw new Error(`Named callout allows only one argument "${a2}"`);
      if (a2.length === 1 && (typeof r5 == "number" || !/^[<>X]$/.test(r5))) throw new Error(`Named callout optional argument must be '<', '>', or 'X' "${r5}"`);
      break;
    case "cmp":
    ...
    default:
      throw new Error(`Unexpected named callout kind "${c2}"`);
  }
  return D(c2, o4 ?? null, (s3 == null ? void 0 : s3.split(",")) ?? null, e);
}
function O(e) {
  let n = null, t, o4;
  if (e[0] === "{") {
    const { minStr: s3, maxStr: a2 } = /^\{(?<minStr>\d*)(?:,(?<maxStr>\d*))?/.exec(e).groups, r5 = 1e5;
    if (+s3 > r5 || a2 && +a2 > r5) throw new Error("Quantifier value unsupported in Oniguruma");
    if (t = +s3, o4 = a2 === void 0 ? +s3 : a2 === "" ? 1 / 0 : +a2, t > o4 && (n = "possessive", [t, o4] = [o4, t]), e.endsWith("?")) {
      if (n === "possessive") throw new Error('Unsupported possessive interval quantifier chain with "?"');
      n = "lazy";
    } else n || (n = "greedy");
  } else t = e[0] === "+" ? 1 : 0, o4 = e[0] === "?" ? 1 : 1 / 0, n = e[1] === "+" ? "possessive" : e[1] === "?" ? "lazy" : "greedy";
  return _(n, t, o4, e);
}
function q(e) {
...
function V(e) {
  const { p: n, neg: t, value: o4 } = /^\\(?<p>[pP])\{(?<neg>\^?)(?<value>[^}]+)/.exec(e).groups;
  return k("property", e, { value: o4, negate: n === "P" && !t || n === "p" && !!t });
}
function v(e) {
  const n = {};
...
function Y(e) {
  ...
  for (let t = 0; t < e.length; t++) {
    const o4 = e[t];
    if (!"imxDPSWy".includes(o4)) throw new Error(`Invalid flag "${o4}"`);
    if (o4 === "y") {
      if (!/^y{[gw]}/.test(e.slice(t))) throw new Error('Invalid or unspecified flag "y" mode');
      n.textSegmentMode = e[t + 2] === "g" ? "grapheme" : "word", t += 3;
      continue;
    }
    n[{ i: "ignoreCase", m: "dotAll", x: "extended", D: "digitIsAscii", P: "posixIsAscii", S: "spaceIsAscii", W: "wordIsAscii" }[o4]] = true;
  }
  return n;
}
function J(e) {
...
function ee(e, n) {
  const { raw: t, inCharClass: o4 } = e, s3 = t.slice(1);
  if (!o4 && (s3 !== "0" && s3.length === 1 || s3[0] !== "0" && +s3 <= n)) return [A(t)];
  const a2 = [], r5 = s3.match(/^[0-7]+|\d/g);
  for (let i2 = 0; i2 < r5.length; i2++) {
    const l3 = r5[i2];
    let c2;
    if (i2 === 0 && l3 !== "8" && l3 !== "9") {
      if (c2 = parseInt(l3, 8), c2 > 127) throw new Error(o`Octal encoded byte above 177 unsupported "${t}"`);
    } else c2 = r(l3);
    a2.push(d(c2, (i2 === 0 ? "\\" : "") + l3));
  }
  return a2;
}
function te(e) {
  ...
  for (; o4 = t.exec(e); ) {
    const s3 = o4[0];
    if (s3[0] === "{") {
      const a2 = /^\{(?<min>\d+),(?<max>\d+)\}\??$/.exec(s3);
      if (a2) {
        const { min: r5, max: i2 } = a2.groups;
        if (+r5 > +i2 && s3.endsWith("?")) {
          t.lastIndex--, n.push(O(s3.slice(0, -1)));
          continue;
        }
      }
    }
...
function o2(e, t) {
  ...
  const r5 = e.body[0];
  return !t || Object.keys(t).every((n) => t[n] === r5[n]);
}
function s(e) {
...
function J2(e, r5 = {}) {
  const n = { flags: "", normalizeUnknownPropertyNames: false, skipBackrefValidation: false, skipLookbehindValidation: false, skipPropertyNameValidation: false, unicodePropertyMap: null, ...r5, rules: { captureGroup: false, singleline: false, ...r5.rules } }, o4 = M(e, { flags: n.flags, rules: { captureGroup: n.rules.captureGroup, singleline: n.rules.singleline } }), i2 = (p3, N) => {
    const u2 = o4.tokens[t.nextIndex];
    switch (t.parent = p3, t.nextIndex++, u2.type) {
      case "Alternator":
        return b2();
      case "Assertion":
        return W2(u2);
  ...
  }, t = { capturingGroups: [], hasNumberedRef: false, namedGroupsByName: /* @__PURE__ */ new Map(), nextIndex: 0, normalizeUnknownPropertyNames: n.normalizeUnknownPropertyNames, parent: null, skipBackrefValidation: n.skipBackrefValidation, skipLookbehindValidation: n.skipLookbehindValidation, skipPropertyNameValidation: n.skipPropertyNameValidation, subroutines: [], tokens: o4.tokens, unicodePropertyMap: n.unicodePropertyMap, walk: i2 }, d2 = B2(T2(o4.flags));
  let s3 = d2.body[0];
  for (; t.nextIndex < o4.tokens.length; ) {
    const p3 = i2(s3, {});
    p3.type === "Alternative" ? (d2.body.push(p3), s3 = p3) : s3.body.push(p3);
  }
  const { capturingGroups: a2, hasNumberedRef: l3, namedGroupsByName: c2, subroutines: f2 } = t;
  if (l3 && c2.size && !n.rules.captureGroup) throw new Error("Numbered backref/subroutine not allowed when using named capture");
  for (const { ref: p3 } of f2) if (typeof p3 == "number") {
    if (p3 > a2.length) throw new Error("Subroutine uses a group number that's not defined");
    p3 && (a2[p3 - 1].isSubroutined = true);
  } else if (c2.has(p3)) {
    if (c2.get(p3).length > 1) throw new Error(o`Subroutine uses a duplicate group name "\g<${p3}>"`);
    c2.get(p3)[0].isSubroutined = true;
...
function X2({ raw: e }, r5) {
  const n = /^\\k[<']/.test(e), o4 = n ? e.slice(3, -1) : e.slice(1), i2 = (t, d2 = false) => {
    const s3 = r5.capturingGroups.length;
    let a2 = false;
    if (t > s3) if (r5.skipBackrefValidation) a2 = true;
    else throw new Error(`Not enough capturing groups defined to the left "${e}"`);
    return r5.hasNumberedRef = true, k2(d2 ? s3 + 1 - t : t, { orphan: a2 });
  };
  if (n) {
    const t = /^(?<sign>-?)0*(?<num>[1-9]\d*)$/.exec(o4);
    if (t) return i2(+t.groups.num, !!t.groups.sign);
    if (/[-+]/.test(o4)) throw new Error(`Invalid backref name "${e}"`);
    if (!r5.namedGroupsByName.has(o4)) throw new Error(`Group name not defined to the left "${e}"`);
    return k2(o4);
  }
  return i2(+o4);
}
function ee2(e, r5, n) {
  const { tokens: o4, walk: i2 } = r5, t = r5.parent, d2 = t.body.at(-1), s3 = o4[r5.nextIndex];
  if (!n.isCheckingRangeEnd && d2 && d2.type !== "CharacterClass" && d2.type !== "CharacterClassRange" && s3 && s3.type !== "CharacterClassOpen" && s3.type !== "CharacterClassClose" && s3.type !== "CharacterClassIntersector") {
...
function re({ negate: e }, r5, n) {
  const { tokens: o4, walk: i2 } = r5, t = [C2()], d2 = o4[r5.nextIndex];
  let s3 = z2(d2);
  for (; s3.type !== "CharacterClassClose"; ) {
    if (s3.type === "CharacterClassIntersector") t.push(C2()), r5.nextIndex++;
    else {
      const l3 = t.at(-1);
      l3.body.push(i2(l3, n));
    }
    s3 = z2(o4[r5.nextIndex], d2);
  }
  const a2 = C2({ negate: e });
  return t.length === 1 ? a2.body = t[0].body : (a2.kind = "intersection", a2.body = t.map((l3) => l3.body.length === 1 ? l3.body[0] : l3)), r5.nextIndex++, a2;
}
function ne({ kind: e, negate: r5, value: n }, o4) {
  const { normalizeUnknownPropertyNames: i2, skipPropertyNameValidation: t, unicodePropertyMap: d2 } = o4;
  if (e === "property") {
    const s3 = w2(n);
    if (i.has(s3) && !(d2 == null ? void 0 : d2.has(s3))) e = "posix", n = s3;
    else return Q2(n, { negate: r5, normalizeUnknownPropertyNames: i2, skipPropertyNameValidation: t, unicodePropertyMap: d2 });
  }
  return e === "posix" ? R2(n, { negate: r5 }) : E2(e, { negate: r5 });
}
function te2(e, r5, n) {
  const { tokens: o4, capturingGroups: i2, namedGroupsByName: t, skipLookbehindValidation: d2, walk: s3 } = r5, a2 = ie(e), l3 = a2.type === "AbsenceFunction", c2 = $2(a2), f2 = c2 && a2.negate;
  ...
  let p3 = D2(o4[r5.nextIndex]);
  for (; p3.type !== "GroupClose"; ) {
    if (p3.type === "Alternator") a2.body.push(b2()), r5.nextIndex++;
    else {
      const N = a2.body.at(-1), u2 = s3(N, { ...n, isInAbsenceFunction: n.isInAbsenceFunction || l3, isInLookbehind: n.isInLookbehind || c2, isInNegLookbehind: n.isInNegLookbehind || f2 });
      if (N.body.push(u2), (c2 || n.isInLookbehind) && !d2) {
    ...
    p3 = D2(o4[r5.nextIndex]);
  }
  return r5.nextIndex++, a2;
}
function oe({ kind: e, min: r5, max: n }, o4) {
  const i2 = o4.parent, t = i2.body.at(-1);
  if (!t || !s(t)) throw new Error("Quantifier requires a repeatable token");
  const d2 = _2(e, r5, n, t);
  return i2.body.pop(), d2;
}
function ae({ raw: e }, r5) {
  const { capturingGroups: n, subroutines: o4 } = r5;
  let i2 = e.slice(3, -1);
  const t = /^(?<sign>[-+]?)0*(?<num>[1-9]\d*)$/.exec(i2);
  if (t) {
    const s3 = +t.groups.num, a2 = n.length;
    if (r5.hasNumberedRef = true, i2 = { "": s3, "+": a2 + s3, "-": a2 + 1 - s3 }[t.groups.sign], i2 < 1) throw new Error("Invalid subroutine number");
  } else i2 === "0" && (i2 = 0);
  const d2 = O2(i2);
  return o4.push(d2), d2;
}
function G(e, r5) {
  if (e !== "repeater") throw new Error(`Unexpected absence function kind "${e}"`);
  return { type: "AbsenceFunction", kind: e, body: h(r5 == null ? void 0 : r5.body) };
}
function b2(e) {
...
function F2(e, r5) {
  ...
  return (e === "word_boundary" || e === "text_segment_boundary") && (n.negate = !!(r5 == null ? void 0 : r5.negate)), n;
}
function k2(e, r5) {
  const n = !!(r5 == null ? void 0 : r5.orphan);
  return { type: "Backreference", ref: e, ...n && { orphan: n } };
}
function P2(e, r5) {
  const n = { name: void 0, isSubroutined: false, ...r5 };
  if (n.name !== void 0 && !se(n.name)) throw new Error(`Group name "${n.name}" invalid in Oniguruma`);
  return { type: "CapturingGroup", number: e, ...n.name && { name: n.name }, ...n.isSubroutined && { isSubroutined: n.isSubroutined }, body: h(r5 == null ? void 0 : r5.body) };
}
function m2(e, r5) {
  const n = { useLastValid: false, ...r5 };
  if (e > 1114111) {
    const o4 = e.toString(16);
    if (n.useLastValid) e = 1114111;
    else throw e > 1310719 ? new Error(`Invalid code point out of range "\\x{${o4}}"`) : new Error(`Invalid code point out of range in JS "\\x{${o4}}"`);
  }
  return { type: "Character", value: e };
}
function C2(e) {
  const r5 = { kind: "union", negate: false, ...e };
  return { type: "CharacterClass", kind: r5.kind, negate: r5.negate, body: V2(e == null ? void 0 : e.body) };
}
function L2(e, r5) {
  if (r5.value < e.value) throw new Error("Character class range out of order");
  return { type: "CharacterClassRange", min: e, max: r5 };
}
function E2(e, r5) {
  const n = !!(r5 == null ? void 0 : r5.negate), o4 = { type: "CharacterSet", kind: e };
  return (e === "digit" || e === "hex" || e === "newline" || e === "space" || e === "word") && (o4.negate = n), (e === "text_segment" || e === "newline" && !n) && (o4.variableLength = true), o4;
}
function I2(e, r5 = {}) {
  if (e === "keep") return { type: "Directive", kind: e };
  if (e === "flags") return { type: "Directive", kind: e, flags: u(r5.flags) };
...
function A2(e) {
  const r5 = e == null ? void 0 : e.atomic, n = e == null ? void 0 : e.flags;
  if (r5 && n) throw new Error("Atomic group cannot have flags");
  return { type: "Group", ...r5 && { atomic: r5 }, ...n && { flags: n }, body: h(e == null ? void 0 : e.body) };
}
function K2(e) {
  const r5 = { behind: false, negate: false, ...e };
  return { type: "LookaroundAssertion", kind: r5.behind ? "lookbehind" : "lookahead", negate: r5.negate, body: h(e == null ? void 0 : e.body) };
}
function U2(e, r5, n) {
  return { type: "NamedCallout", kind: e, tag: r5, arguments: n };
}
function R2(e, r5) {
  const n = !!(r5 == null ? void 0 : r5.negate);
  if (!i.has(e)) throw new Error(`Invalid POSIX class "${e}"`);
  return { type: "CharacterSet", kind: "posix", value: e, negate: n };
}
function _2(e, r5, n, o4) {
  if (r5 > n) throw new Error("Invalid reversed quantifier range");
  return { type: "Quantifier", kind: e, min: r5, max: n, body: o4 };
}
function B2(e, r5) {
  return { type: "Regex", body: h(r5 == null ? void 0 : r5.body), flags: e };
}
function O2(e) {
...
function Q2(e, r5) {
  var _a4;
  const n = { negate: false, normalizeUnknownPropertyNames: false, skipPropertyNameValidation: false, unicodePropertyMap: null, ...r5 };
  let o4 = (_a4 = n.unicodePropertyMap) == null ? void 0 : _a4.get(w2(e));
  if (!o4) {
    if (n.normalizeUnknownPropertyNames) o4 = de(e);
    else if (n.unicodePropertyMap && !n.skipPropertyNameValidation) throw new Error(o`Invalid Unicode property "\p{${e}}"`);
  }
  return { type: "CharacterSet", kind: "property", value: o4 ?? e, negate: n.negate };
}
function ie({ flags: e, kind: r5, name: n, negate: o4, number: i2 }) {
  switch (r5) {
    ...
    case "lookahead":
    case "lookbehind":
      return K2({ behind: r5 === "lookbehind", negate: o4 });
    default:
      throw new Error(`Unexpected group kind "${r5}"`);
  }
}
function h(e) {
  if (e === void 0) e = [b2()];
  else if (!Array.isArray(e) || !e.length || !e.every((r5) => r5.type === "Alternative")) throw new Error("Invalid body; expected array of one or more Alternative nodes");
  return e;
}
function V2(e) {
  if (e === void 0) e = [];
  else if (!Array.isArray(e) || !e.every((r5) => !!r5.type)) throw new Error("Invalid body; expected array of nodes");
  return e;
}
function M2(e) {
...
function de(e) {
  return e.trim().replace(/[- _]+/g, "_").replace(/[A-Z][a-z]+(?=[A-Z])/g, "$&_").replace(/[A-Za-z]+/g, (r5) => r5[0].toUpperCase() + r5.slice(1).toLowerCase());
}
function w2(e) {
...
function S(a2, v3, N = null) {
  function b3(e, s3) {
    for (let t = 0; t < e.length; t++) {
      const r5 = n(e[t], s3, t, e);
      t = Math.max(-1, t + r5);
    }
  }
  function n(e, s3 = null, t = null, r5 = null) {
    var _a4, _b;
    let i2 = 0, c2 = false;
    const d2 = { node: e, parent: s3, key: t, container: r5, root: a2, remove() {
      x2(r5).splice(Math.max(0, l2(t) + i2), 1), i2--, c2 = true;
    }, removeAllNextSiblings() {
      return x2(r5).splice(l2(t) + 1);
    }, removeAllPrevSiblings() {
      const o4 = l2(t) + i2;
      return i2 -= o4, x2(r5).splice(0, Math.max(0, o4));
    }, replaceWith(o4, m3 = {}) {
      const y3 = !!m3.traverse;
      r5 ? r5[Math.max(0, l2(t) + i2)] = o4 : u(s3, "Can't replace root node")[t] = o4, y3 && n(o4, s3, t, r5), c2 = true;
    }, replaceWithMultiple(o4, m3 = {}) {
      const y3 = !!m3.traverse;
      if (x2(r5).splice(Math.max(0, l2(t) + i2), 1, ...o4), i2 += o4.length - 1, y3) {
        let g = 0;
        for (let p3 = 0; p3 < o4.length; p3++) g += n(o4[p3], s3, l2(t) + p3 + g, r5);
      }
      c2 = true;
    }, skip() {
      c2 = true;
    } }, { type: f2 } = e, u2 = v3["*"], h2 = v3[f2], R3 = typeof u2 == "function" ? u2 : u2 == null ? void 0 : u2.enter, P4 = typeof h2 == "function" ? h2 : h2 == null ? void 0 : h2.enter;
    if (R3 == null ? void 0 : R3(d2, N), P4 == null ? void 0 : P4(d2, N), !c2) switch (f2) {
    ...
    return (_a4 = h2 == null ? void 0 : h2.exit) == null ? void 0 : _a4.call(h2, d2, N), (_b = u2 == null ? void 0 : u2.exit) == null ? void 0 : _b.call(u2, d2, N), i2;
  }
  return n(a2), a2;
}
function x2(a2) {
  ...
  Assertion({ node, parent, key: key2, container, root: root2, remove, replaceWith }, state) {
    ...
    if (kind === "text_segment_boundary") {
      ...
      if (!wordIsAscii && !asciiWordBoundaries) {
        ...
        replaceWith(setParentDeep(parseFragment(negate ? B4 : b3), parent));
      }
    } else {
...
var JavaScriptScanner = class {
  constructor(patterns, options = {}) {
    ...
    this.regexps = patterns.map((p3) => {
      ...
      const cached = cache == null ? void 0 : cache.get(p3);
      if (cached) {
      ...
      try {
        ...
        cache == null ? void 0 : cache.set(p3, regex);
        return regex;
      } catch (e) {
        cache == null ? void 0 : cache.set(p3, e);
        if (forgiving)
          return null;
        throw e;
      }
...
var S2 = createJavaScriptRegexEngine({ forgiving: true });
var C3 = Object.fromEntries(bundledLanguagesInfo.flatMap((e) => {
  var n;
  return ((n = e.aliases) != null ? n : []).map((t) => [t, e.id]);
}));
var r4 = new Set(Object.keys(bundledLanguages));
var B3 = (e) => {
  let t = e.trim().toLowerCase(), g = C3[t];
  return g || (r4.has(t), t);
};
...
var v2 = (e, n) => `${e}-${o3(n[0])}-${o3(n[1])}`;
var x3 = (e, n, t) => {
  let g = e.slice(0, 100), u2 = e.length > 100 ? e.slice(-100) : "";
  return `${n}:${t[0]}:${t[1]}:${e.length}:${g}:${u2}`;
};
var P3 = (e, n) => {
  let t = v2(e, n);
  if (c.has(t)) return c.get(t);
  let g = createHighlighter({ themes: n, langs: [e], engine: S2 });
  return c.set(t, g), g;
};
function $3(e = {}) {
  var t;
  ...
  return { name: "shiki", type: "code-highlighter", supportsLanguage(g) {
    ...
    return n;
  }, highlight({ code: g, language: u2, themes: h2 }, m3) {
    let i2 = B3(u2), d2 = [o3(h2[0]), o3(h2[1])], a2 = x3(g, i2, d2);
    if (p2.has(a2)) return p2.get(a2);
    m3 && (s2.has(a2) || s2.set(a2, /* @__PURE__ */ new Set()), s2.get(a2).add(m3));
    let f2 = r4.has(i2) ? i2 : "text";
    return P3(f2, h2).then((l3) => {
      let y3 = l3.getLoadedLanguages().includes(i2) ? i2 : "text", L3 = l3.codeToTokens(g, { lang: y3, themes: { light: d2[0], dark: d2[1] } });
      p2.set(a2, L3);
      let T3 = s2.get(a2);
      if (T3) {
        for (let H2 of T3) H2(L3);
        s2.delete(a2);
      }
    }).catch((l3) => {
      console.error("[Streamdown Code] Failed to highlight code:", l3), s2.delete(a2);
    }), null;
  } };
}
var G2 = $3();
